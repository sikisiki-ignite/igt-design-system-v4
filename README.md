...
### 정적 HTML로 페이지를 만들 때 (React 없이)

AI가 React 컴포넌트 없이 단일 HTML 파일로 페이지를 만들 때는 `dist/html-components.html`을 참조하라고 알려주세요. 이 파일은 모든 IGT 컴포넌트의 정확한 HTML 구조를 복사 가능한 형태로 담고 있습니다 (build 시 자동 생성).

```
명의이전 조회 페이지를 정적 HTML로 만들어줘.
컴포넌트 구조는 dist/html-components.html을 참조해서 정확히 사용해.
체크박스, 셀렉트, 텍스트필드는 절대 native input으로 도망가지 말고
IGT 컴포넌트 구조를 그대로 복사해.
```
