...
### 3단계. 새 프로젝트 시작 (React + Vite 환경)

IGT 디자인 시스템은 **React + Vite 환경**에서 작동합니다. 정적 HTML 파일로 페이지를 만들면 컴포넌트 캡슐화·토큰 강제·레이아웃 패턴이 무너지므로(관련 사고 이력 있음), 페이지 단위 결과물은 React로 작성하세요. 빌드 환경이 없으면 Claude에게 셋업까지 시키면 됩니다.

```
새 폴더 만들고 Vite + React + TypeScript 환경 세팅한 다음
igt-design-system-v4를 GitHub에서 npm install해줘.
```

`npm install` 시 프로젝트 루트에 `CLAUDE.md`와 `.claude/commands/backoffice-dev.md`가 **자동 생성**됩니다. 별도 복사 작업 불필요. Claude는 이 두 파일을 읽고 자동으로 디자인 시스템 규칙을 따릅니다.
...
`showcase/TitleTransferPage.tsx` + `showcase/TitleTransferPage.css` — 백오피스 페이지 **정답 샘플**. 토큰 매핑(페이지 외곽=`--sys-surface-static`, LNB=`--sys-background-subtle`, 컨텐츠=`--sys-background-base`, 섹션=`--sys-background-subtle`), 레이아웃, 컴포넌트 조합 패턴이 모두 들어 있습니다. 새 페이지 만들기 전에 Claude에게 이 파일을 먼저 읽고 같은 패턴을 따르라고 지시하세요.
